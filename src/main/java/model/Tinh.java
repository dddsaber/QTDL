...
public class Tinh implements Serializable {
	...
	public static ArrayList<Tinh> getDSTinh() {
		...
		for(String tenTinh : arr_tinh) {
			Tinh tmp = new Tinh(i++, tenTinh);
			listTinh.add(tmp);
		}
		return listTinh;
	}
	public static Tinh getTinhbyTen(String s) {
		ArrayList<Tinh> tmp = getDSTinh();
		for(Tinh tinh : tmp) {
			tinh.setMaTinh(tmp.indexOf(tinh));
			if(tinh.tenTinh.equals(s))
				return tinh;
		}
		return null;
	}
	public static Tinh getTinhbyId(int matinh) {
		ArrayList<Tinh> tmp = getDSTinh();
		for(Tinh t : tmp) {
			if(t.maTinh == matinh)
				return t;
