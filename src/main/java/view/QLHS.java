...
//		Các tab bắt đầu từ đây
		...
		tableThongTinGV.setModel(new DefaultTableModel(
			new Object[][] {
			},
			new String[] {
				"STT", "M\u00E3 gi\u00E1o vi\u00EAn", "H\u1ECD t\u00EAn", "Ng\u00E0y sinh", "\u0110\u1ECBa ch\u1EC9", "S\u1ED1 \u0111i\u1EC7n tho\u1EA1i"
			}
		));
		...
		tableChuNhiem.setModel(new DefaultTableModel(
			new Object[][] {
			},
			new String[] {
				"Mã giáo viên", "Mã lớp", "Năm học"
			}
		));
		...
		/*
		 * Tab giáo viên kết thúc ở đây
		 * TEACHER END
		 */
		
		
		/*
		 * Tab học sinh bắt đầu từ đây
		 * STUDENT START
		 */
...
//		panelStudent.add(panelStudent);
		...
		tableHocSinh.setModel(new DefaultTableModel(
			new Object[][] {
			},
			new String[] {
				"Mã học sinh", "Họ tên", "Ngày sinh", "Địa chỉ", "SDT Phụ huynh", "Mã lớp", "Tên lớp"
			}
		));
		...
		/*
		 * Tab học sinh kết thúc ở đây
		 * STUDENT END
		 */
		
		
		/*
		 * Tab quản lý điểm bắt đầu từ đây
		 * SCORE START
		 */
		...
		tableThongTinDiem.setModel(new DefaultTableModel(
			...
			new String[] {
				"Mã sinh viên", "Tên sinh viên", "Tên môn", "điểm miệng", "15 phút", "1 tiết", "Học kỳ"
			}
		));
		...
		JButton btnCapNhatGV_1_1 = new JButton("Cập nhật");
		btnCapNhatGV_1_1.setBounds(140, 103, 89, 23);
		panelScore.add(btnCapNhatGV_1_1);
		...
		/*
		 * Tab quản lý điểm kết thúc ở đây
		 *  SCORE END
		 */
		
		
		/*
		 * Tab quản lý phòng học bắt đầu từ đây
		 * CLASSROOM START
		 */
		...
		tableThongTinPhong.setModel(new DefaultTableModel(
			new Object[][] {
			},
			new String[] {
				"Mã phòng", "Số phòng", "Số chỗ tối đa"
			}
		));
		...
		tableThongTinLop.setModel(new DefaultTableModel(
			new Object[][] {
			},
			new String[] {
				"Mã phòng", "Mã lớp", "Học kỳ - năm học"
			}
		));
		...
		/*
		 * Tab quản lý phòng học kết thúc ở đây
		 * CLASSROOM END
		 */
	...
	public void luuDuLieuGVtuInput() {
		//Lay du lieu giao vien tu cac textField va textArea de thuc thi cac thao tac them/sua dl
	...
	private void themGVvaoBangDL(GiaoVien gv) {
		...
		mode.addRow(new Object[]{
				this.gvModel.getDsGiaoVien().lastIndexOf(gv)+1,
				gv.getMaGV(),
				gv.getHoTenGV(),
	...
	public void xoaGV() {
		DefaultTableModel mode = (DefaultTableModel) this.tableThongTinGV.getModel();
		int i_row = this.tableThongTinGV.getSelectedRow();
		int luaChon = JOptionPane.showConfirmDialog(this, "Bạn có chắc muốn xoá giáo viên này ra khỏi cơ sở dữ liệu không?");
